...
describe("Test POST /REGISTER", function () { // DEKLARASI FUNCTION YANG AKAN DI TES
  ...
  it("Failed regis jika email Kosong", async function () {
  ...
  it("Failed regis jika name Kosong", async function () {
    let random_email = Math.random().toString(36).substring(7);
    const response = await request_url
      .post("/register")
  ...
  it("Failed regis jika password Kosong", async function () {
    let random_email = Math.random().toString(36).substring(7);
    const response = await request_url
      .post("/register")
      .send({ email: random_email + "@gmail.com", password: "", name: "jago" });
    const hasil_response = response.body;
    assert(response.body.status).to.eql('FAILED_REGISTER');
    assert(response.body.data).to.eql('Email/Username/Password tidak boleh kosong');
    assert(hasil_response).to.include.keys("data", "message", "status"); 
  ...
  it("Failed regis jika  password melebihi max char", async function () {
  ...
  it("Failed regis jika  email melebihi max char", async function () {
    let random_email = Array.from(Array(55), () => Math.floor(Math.random() * 36).toString(36)).join('');
    let random_pass = Array.from(Array(11), () => Math.floor(Math.random() * 36).toString(36)).join('');
    const response = await request_url
      .post("/register")
  ...
  it("Failed regis jika email tidak sesuai format", async function () {
    const response = await request_url
      .post("/register")
      .send({ email: "tester@@gmail.com", password: "aditya.qa", name: "Test Email Gmail"});
    const hasil_response = response.body;
    assert(response.body.status).to.eql('FAILED_REGISTER');
    assert(response.body.data).to.eql('Email sudah terdaftar ,Cek kembali email anda');
    assert(hasil_response).to.include.keys("data", "message", "status"); 
  });


  it("Failed regis jika name menggunakan int", async function () {
    const response = await request_url
      .post("/register")
      .send({ email: "tester@@gmail.com", password: "aditya.qa", name: "12345"});
    const hasil_response = response.body;
    assert(response.body.status).to.eql('FAILED_REGISTER');
    assert(response.body.data).to.eql('Email sudah terdaftar ,Cek kembali email anda');
    assert(hasil_response).to.include.keys("data", "message", "status"); 
